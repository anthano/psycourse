...
def univariate_lipid_regression_cov_panss(multimodal_df):
    ...
    for lipid in lipid_columns:
        ...
        formula = (
            f"prob_class_5 ~ {lipid} + age + C(sex) + bmi + "
            "duration_illness + C(smoker) + panss_sum_pos"
        )
...
def univariate_lipid_regression_cov_panss_gen(multimodal_df):
    """
    Perform univariate regression analysis for lipid intensity values against
    the probability of class 5 with an added covariate panss gen.
    This function iterates through all lipid columns in the
    provided multimodal DataFrame, fits a GLM model for each, and returns a DataFrame
    with coefficients, p-values, and FDR-corrected p-values.
    ...
    for lipid in lipid_columns:
        ...
        formula = (
            f"prob_class_5 ~ {lipid} + age + C(sex) + bmi + "
            "duration_illness + C(smoker) + panss_sum_gen"
        )
...
def univariate_lipid_regression_cov_panss_total(multimodal_df):
    """
    Perform univariate regression analysis for lipid intensity values against
    the probability of class 5 with an added covariate panss total.
    This function iterates through all lipid columns in the
    provided multimodal DataFrame, fits a GLM model for each, and returns a DataFrame
    with coefficients, p-values, and FDR-corrected p-values.
    ...
    for lipid in lipid_columns:
        ...
        formula = (
            f"prob_class_5 ~ {lipid} + age + C(sex) + bmi + "
            "duration_illness + C(smoker) + panss_total_score"
        )
...
def univariate_lipid_regression_cov_panss_pos_neg(multimodal_df):
    """
    Perform univariate regression analysis for lipid intensity values against
