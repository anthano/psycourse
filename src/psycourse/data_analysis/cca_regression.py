...
def cca_regression_analysis(
    ...
    p_perm_prs, null_t_prs = _permute_regression_pval(
        scores_df,
        y_col,
        "PRS_CCA_Component_1",
        t_prs,
    ...
    p_perm_lip, null_t_lip = _permute_regression_pval(
        scores_df,
        y_col,
        "Lipid_CCA_Component_1",
        t_lip,
        n_permutations=n_perm_regression,
        random_state=random_state,
        return_null=True,
    ...
    results_dict = {
        ...
        "prs_regression_null_t": null_t_prs,
        "lipid_regression_null_t": null_t_lip,
...
def _permute_regression_pval(
    ...
    """
    Permutation test for y ~ score (with intercept).
    Null generated by permuting y across rows.
    Test statistic is the usual OLS t for the slope, computed via r->t identity.
    """
    x = scores_df[score_col].to_numpy(dtype=float)
    y = scores_df[y_col].to_numpy(dtype=float)
    ...
    if n < 3 or not np.isfinite(t_observed):
        return (
            (float("nan"), np.array([], dtype=float)) if return_null else float("nan")
        )

    # center x once
    x0 = x - x.mean()
    x_ss = np.sqrt(np.dot(x0, x0))
    if x_ss == 0.0:
        null_t = np.zeros(n_permutations, dtype=float)
        p = float(
            (1 + np.sum(np.abs(null_t) >= abs(float(t_observed))))
            / (1 + n_permutations)
        )
        return (p, null_t) if return_null else p

    rng = np.random.default_rng(random_state)
    null_t = np.empty(n_permutations, dtype=float)

    for i in range(n_permutations):
        yp = y[rng.permutation(n)]
        y0 = yp - yp.mean()
        y_ss = np.sqrt(np.dot(y0, y0))

        if y_ss == 0.0:
            r = 0.0
        else:
            r = float(np.dot(x0, y0) / (x_ss * y_ss))

        # numerical guard
        r = min(0.9999999999, max(-0.9999999999, r))
        null_t[i] = r * np.sqrt((n - 2) / (1.0 - r * r))

    p = float(
        (1 + np.sum(np.abs(null_t) >= abs(float(t_observed)))) / (1 + n_permutations)
    )
    return (p, null_t) if return_null else p
